...
Approach:
  1. Parse shapes with python-pptx (geometry, fills, borders, text runs, images, tables).
...
def color_of(color_format, theme):
    ...
    try:
        ...
        if str(color_format.type).startswith("MSO_THEME_COLOR"):
            return theme.get(color_format.theme_color, None)
...
def read_theme(prs):
    """Extract theme palette and fonts from theme1.xml."""
    ...
    for rel in part.rels.values():
        if "theme" in rel.reltype:
            ...
            for name in ("dk1", "lt1", "dk2", "lt2", "accent1", "accent2",
                ...
                m = re.search(rf'<a:{name}>.*?(?:val|lastClr)="([0-9A-Fa-f]{{6}})"', xml, re.S)
                if m:
                    theme["colors"][name] = "#" + m.group(1).upper()
            m = re.search(r'<a:majorFont><a:latin typeface="([^"]*)"', xml)
            theme["major_font"] = m.group(1) if m else "Calibri Light"
            m = re.search(r'<a:minorFont><a:latin typeface="([^"]*)"', xml)
            theme["minor_font"] = m.group(1) if m else "Calibri"
    return theme


def detect_connector(shape, d):
    """Detect line/connector shapes (flow arrows) from their XML geometry."""
    try:
        xml = shape._element.spPr.xml
    except AttributeError:
        return False
    is_line = bool(CONNECTOR_PRST.search(xml))
    if not is_line and shape.shape_type == MSO_SHAPE_TYPE.LINE:
        is_line = True
    if not is_line:
        return False
    d["kind"] = "connector"
    ...
    d["stroke"] = "#" + m.group(1).upper() if m else "#444444"
    d["arrow_start"] = bool(re.search(r'<a:headEnd type="(?!none)', xml))
    d["arrow_end"] = bool(re.search(r'<a:tailEnd type="(?!none)', xml))
    if not (d["arrow_start"] or d["arrow_end"]):
        d["arrow_end"] = True   # plain flow line: still draw direction
    return True
...
def run_style(run, theme):
    ...
    return {
        ...
        "bold": f.bold,
        "italic": f.italic,
        "color": color_of(f.color, theme.get("colors", {})),
    }


def extract_text(shape, theme):
    paras = []
    for p in shape.text_frame.paragraphs:
        runs = [{"text": r.text, **run_style(r, theme)} for r in p.runs if r.text]
        ls = p.line_spacing
    ...
    for r in shape.table.rows:
        ...
        for c in r.cells:
            row.append({
                ...
                "fill": color_of(c.fill.fore_color, theme["colors"])
...
def extract_shape(shape, theme, assets_dir, asset_list):
    d = {
    ...
    try:
        if shape.fill.type is not None and shape.fill.type == 1:  # solid
            d["fill"] = color_of(shape.fill.fore_color, theme["colors"])
    except (AttributeError, TypeError):
        pass
    try:
        line = shape.line
        if line.fill.type is not None and line.fill.type == 1 and line.width:
            d["border"] = {"color": color_of(line.color, theme["colors"]),
                           "width": px(line.width)}
    except (AttributeError, TypeError):
        pass
    ...
    if shape.has_text_frame and shape.text_frame.text.strip():
        d["kind"] = "text"
        d["paras"] = extract_text(shape, theme)
        d["anchor"] = ANCHOR_MAP.get(shape.text_frame.vertical_anchor)
    return d


def flatten(shapes, theme, assets_dir, asset_list):
    out = []
    for sh in shapes:
        if sh.shape_type == MSO_SHAPE_TYPE.GROUP:
            out.extend(flatten(sh.shapes, theme, assets_dir, asset_list))
        else:
            try:
                out.append(extract_shape(sh, theme, assets_dir, asset_list))
...
def render_flow_row(row, cells):
    """A row containing connectors: boxes flex, arrows sit between them."""
    parts = []
...
def convert(pptx_path, out_dir, slide_index=0):
    ...
    assets = []
    shapes = flatten(slide.shapes, theme, assets_dir, assets)
    ...
    for row in rows:
        ...
        if any(c["kind"] == "connector" for c in row):
            ...
            body_rows.append(
                f'<div class="grid-row" style="margin-top:{max(gap, 0)}px; --gap:{gap_x}px">'
...
html {{ font-size: 16px; }}
body {{ font-family: var(--font-body); background: #fff; color: #000; }}
